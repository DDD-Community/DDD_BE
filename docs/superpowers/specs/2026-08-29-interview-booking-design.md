...
전이 흐름이 `서류심사대기 → 서류합격 → 면접합격 → 최종합격 → 활동중` 이라,
지원자에게 합격을 알리는 시점은 `면접합격` 이다. `최종합격` 은 운영진이 참가비 입금과
참여 의사를 확인해 올리는 내부 단계라 메일을 보내지 않는다.

| 상태 | 메일 |
|---|---|
| 서류합격 | 서류 합격 및 면접 일정 선택 안내 (예약 링크 포함) |
| 서류불합격 | 서류 전형 결과 안내 |
| 면접합격 | **최종 합격 및 참가 안내** (참가비·입금 계좌·기한) |
| 최종합격 | 없음 |
| 최종불합격 | 면접 전형 결과 안내 |

면접 슬롯을 예약하면 위와 별개로 `면접 일정 확정 안내` 메일이 나간다.
